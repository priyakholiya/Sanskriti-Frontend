...
const Sarees = () => {
  ...
  useEffect(() => {
    // In a real app, we would fetch by category. Tracking all for now.
    productsApi.getAvailable()
      .then(data => setProducts(data.filter(p => p.category === 'SAREE')))
      .catch(console.error)
  ...
  return (
    <div style={{ backgroundColor: 'var(--bg-ivory)', minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        <img src="/images/saree.png" alt="Saree Hero" style={{ position: 'absolute', width: '100%', height: '100%', objectFit: 'cover', filter: 'brightness(0.8)' }} />
        <div style={{ position: 'relative', zIndex: 10, textAlign: 'center', color: '#fff' }}>
          <h4 className="gold-caption" style={{ color: 'var(--muted-gold)' }}>Royal Drapes</h4>
          <h1 className="mega-title" style={{ fontSize: '4rem' }}>Sarees</h1>
        </div>
      </div>

      <div style={{ padding: '80px 40px', maxWidth: '1400px', margin: '0 auto' }}>
        {loading ? (
          <div style={{ textAlign: 'center', padding: '100px 0', fontFamily: 'Playfair Display', fontSize: '2rem' }}>Curating Sarees...</div>
        ) : (
